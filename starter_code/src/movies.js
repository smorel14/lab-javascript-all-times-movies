...
function turnHoursToMinutes(movies){
  return  movies.map(movie =>  {
    ...
    let duration;
      if(str.length === "Xh XXmin".length){
        duration = str[0]* 60 + str[3]*10 + str[4]*1;
      }
      if(str.length === "xh xmin".length){
          duration = str[0] * 60 + str[2]*1;
      }
      if (str.length === "xh". length){
          duration = str[0]*60;
      }
      if(str.length === "xxmin".length){
          duration = str[0]*10 + str[1]*1;
      }
...
function ratesAverage(movies){
    let total = movies.reduce((acc,y) => acc + parseFloat(y.rate), 0);
    //console.log(total);
...
function dramaMoviesRate(movies){
    ...
    for(i=0; i<movies.length; i++){
        if(movies[i].genre.includes('Drama') && movies[i].rate.length !== 0){
            drama.push(movies[i].rate);
        }
        
        else if(movies[i].genre.includes('Drama') && movies[i].rate.length === 0){
            drama.push(0);
        }
    }
    //console.log('drama' + drama);

    let total = drama.reduce((acc,y) => acc + parseFloat(y), 0);
    if(drama.length === 0){
        return undefined;
    }
    return parseFloat((total / drama.length).toFixed(2));
}
console.log('average rate' + dramaMoviesRate(movies));
...
function orderByDuration (movies){
    ...
    else if(a.duration === b.duration && a.title < b.title) return -1
    else if(a.duration === b.duration && a.title > b.tible) return 1})
